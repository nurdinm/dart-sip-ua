...
class SIPUAHelper extends EventManager {
  ...
  Future<void> start(UaSettings uaSettings) async {
    ...
    if (uaSettings.transportType == TransportType.TCP) {
      SIPUATcpSocket socket = SIPUATcpSocket(uaSettings.host ?? '0.0.0.0', uaSettings.port ?? '5060',
          messageDelay: 1, tcpSocketSettings: uaSettings.tcpSocketSettings);
      _settings.sockets!.add(socket);
    }

    if (uaSettings.transportType == TransportType.WS || uaSettings.transportType == TransportType.WSS) {
      SIPUAWebSocket socket = SIPUAWebSocket(uaSettings.webSocketUrl ?? 'wss://tryit.jssip.net:10443',
          messageDelay: _settings.sip_message_delay, webSocketSettings: uaSettings.webSocketSettings);
      _settings.sockets!.add(socket);
    }

    _settings.transportType = uaSettings.transportType!;
    _settings.uri = uaSettings.uri != null ? Utils.normalizeTarget(uaSettings.uri!) : null;
    ...
    _settings.contact_uri = uaSettings.contact_uri != null ? Utils.normalizeTarget(uaSettings.contact_uri!) : null;
    ...
    try {
      ...
      _ua!.on(EventNewRTCSession(), (EventNewRTCSession event) {
        ...
        _notifyCallStateListeners(event, CallState(CallStateEnum.CALL_INITIATION, video: session.data?['video']));
      });
  ...
  Map<String, dynamic> _options([bool voiceonly = false]) {
    ...
    handlers.on(EventCallProgress(), (EventCallProgress event) {
      ...
      _notifyCallStateListeners(
          event, CallState(CallStateEnum.PROGRESS, originator: event.originator, cause: event.cause));
    });
    ...
    handlers.on(EventStream(), (EventStream event) async {
      ...
      Timer(Duration(milliseconds: 100), () {
        _notifyCallStateListeners(
            event, CallState(CallStateEnum.STREAM, stream: event.stream, originator: event.originator));
      });
    });
    ...
    Map<String, dynamic> defaultOptions = <String, dynamic>{
      ...
      'pcConfig': <String, dynamic>{
        ...
        'iceTransportPolicy': (_uaSettings?.iceTransportPolicy ?? IceTransportPolicy.ALL).toParameterString(),
